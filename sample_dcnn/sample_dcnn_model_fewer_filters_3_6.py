...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct  5 12:30:08 2018
...
class Model(nn.Module):
    def __init__(self,segments, samples, num_labels):
        super().__init__()
        initial_filter_length = 81
        filter_length = 3
        # depth = 6
        padding = int(round((filter_length-1)/2))
        modules = []
        
        modules.append(nn.Conv1d(segments,128,initial_filter_length,stride=initial_filter_length,padding=int(round((initial_filter_length-1)/2))))
